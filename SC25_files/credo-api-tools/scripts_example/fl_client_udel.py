#!/usr/bin/env python3
"""
Federated Learning Client for University of Delaware
...
# UDel-specific configuration
INSTITUTION_NAME = os.getenv("INSTITUTION_NAME", "University of Delaware")
INSTITUTION_SITE = os.getenv("INSTITUTION_SITE", "udel")
UDEL_CLUSTERS = [7, 8, 9]  # UDel's assigned clusters

class UDelFLClient(fl.client.NumPyClient):
    """Federated Learning Client for University of Delaware"""
    
    def __init__(self):
        self.institution_name = INSTITUTION_NAME
        self.cluster_ids = UDEL_CLUSTERS
        self.model = None
        ...
        # Load UDel-specific data
        self._load_institution_data()
        self._build_model()
    
    def _load_institution_data(self):
        """Load images for UDel's clusters (training) and all clusters (testing)"""
        ...
        # Step 2: Filter training data to only UDel's clusters
    ...
    def fit(self, parameters: List[np.ndarray], config: Dict) -> Tuple[List[np.ndarray], int, Dict]:
        """Train the model on local UDel data"""
