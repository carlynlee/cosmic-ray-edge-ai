...
def process_detections(detections, data_dir):
    ...
    # Use detection 'id' field as Elasticsearch document '_id' to ensure uniqueness
    # op_type='create' prevents overwriting existing documents (idempotent)
    # If a document with the same _id already exists, it will be skipped
    bulk(
        es,
        transform_detections(detections),
        index=DETECTION_INDEX_NAME,
        id_field='id',  # Use detection 'id' field as document '_id'
        op_type='create',  # Only create if doesn't exist, skip if exists (prevents duplicates)
